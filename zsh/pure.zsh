...
prompt_pure_precmd() {
    ...
    ns="$(kubectl config view -o "jsonpath={.contexts[?(@.name==\"$cluster\")].context.namespace}")"
    [[ -z "$ns" ]] && ns="default"
    aws=""

    if [[ "$AWS_ACCESS_KEY_ID" != "" ]]; then
       aws=${accounts[$AWS_ACCESS_KEY_ID]}
       if [[ "$aws" == "" ]]; then
        aws=$(get_current_account)
        accounts[$AWS_ACCESS_KEY_ID]=$aws
       fi
    fi
    RPROMPT='$aws|$cluster/$ns'
}
